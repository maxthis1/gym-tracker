...
// Compact muscle map — front + back side by side, no labels, small size
...
export function MuscleMapMini({ primaryMuscle, secondaryMuscles }: Props) {
  const primaryIds = new Set([
    ...(FRONT_MAP[primaryMuscle] ?? []),
    ...(BACK_MAP[primaryMuscle] ?? []),
  ]);
  const secondaryIds = new Set(
    secondaryMuscles.flatMap((m) => [
      ...(FRONT_MAP[m] ?? []),
      ...(BACK_MAP[m] ?? []),
    ])
  );
  ...
  const color = (id: string) => {
    if (primaryIds.has(id))   return "hsl(4 90% 58%)";
    if (secondaryIds.has(id)) return "hsl(25 95% 60%)";
    return "transparent";
  };
  ...
  const renderPaths = (ids: string[]) =>
    ids.map((id) => {
      const d = PATHS[id];
      const fill = color(id);
      if (!d || fill === "transparent") return null;
      return <path key={id} d={d} fill={fill} opacity={0.9} />;
    });

  return (
    <div className="flex items-center gap-0.5 shrink-0">
      <svg viewBox="0 0 80 180" width="28" height="56" className="text-foreground">
        <Silhouette />
        {renderPaths(frontIds)}
      </svg>
      <svg viewBox="0 0 80 180" width="28" height="56" className="text-foreground">
        <Silhouette />
        {renderPaths(backIds)}
